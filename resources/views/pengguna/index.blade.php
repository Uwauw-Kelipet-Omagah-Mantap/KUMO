    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                            ...
                            <tbody>
                                ...
                                @foreach ( $user as $pengguna )
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#editAkunModal{{ $pengguna->id }}">Edit</button>

                                            <form action="{{ url('/admin', ['manage-user', 'destroy', $pengguna->username]) }}" method="POST" class="d-inline">
                                                @method('DELETE')
                                                @csrf
                ...
                <!-- Modal -->
    <div class="modal fade" id="editAkunModal{{ $pengguna->id }}" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editAkunModalLabel{{ $pengguna->id }}">Edit Akun</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <!-- Form Edit Mobil -->
                    <form method="post" action="{{ route('pengguna.update', $pengguna->username) }}">
                        @csrf
                        <div class="mb-3">
                            <label for="" class="form-label">Username</label>
                            <input type="text" name="username" class="form-control mb-3" placeholder="Masukkan Username" value="{{old('username') ?? $pengguna->username }}">
                            </select>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="role" class="form-select mb-3">
                                ...
                                <option value="pemilik_mobil" {{ $pengguna->role === 'pemilik_mobil' ? 'selected' : '' }}>Pemilik Mobil</option>
